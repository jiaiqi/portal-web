...
redis:
  ...
  port: 6379
  db: 0
  keyPrefix: 'nest-admin:'

# JWT 配置
jwt:
  secretkey: 'your_very_long_random_secret_key_change_in_production'
  expiresin: '24h'
  refreshExpiresIn: '7d'
```
...
```bash
# 如果是首次部署，需要导入数据库
mysql -u nest_admin -p nest_admin < /var/www/nest-admin/server/db/nest_admin.sql

# 如果需要添加菜单数据
mysql -u nest_admin -p nest_admin < /var/www/nest-admin/server/db/add_article_audit_menu.sql
mysql -u nest_admin -p nest_admin < /var/www/nest-admin/server/db/add_auditor_user.sql
```
...
# 检查数据库连接
mysql -u nest_admin -p -h localhost nest_admin
...
# 修改配置文件
sed -i "s/your_mysql_password/${DB_PASS}/g" src/config/prod.yml
sed -i "s/your_redis_password/${REDIS_PASS}/g" src/config/prod.yml
sed -i "s/your_very_long_random_secret_key_change_in_production/${JWT_SECRET}/g" src/config/prod.yml
sed -i "s/your-domain.com/${DOMAIN}/g" src/config/prod.yml
